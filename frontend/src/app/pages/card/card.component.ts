...
export class CardComponent {
  baseURL: string = 'http://23.82.141.146:3002/cards';
  ...
  uploadListener($event: any): void {
    ...
    if (this.isValidCSVFile(files[0])) {
      ...
      reader.onload = () => {
        ...
        this.http.post(this.baseURL + '/upload', {cards: this.csvResult})
          .subscribe(data => {
  ...
  getDataRecordsArrayFromCSVFile(csvRecordsArray: any, headerLength: any) {
    ...
    for (let i = 1; i < csvRecordsArray.length; i++) {
      ...
      if (curruntRecord.length == headerLength) {
        ...
        csvRecord.cardNo = curruntRecord[0].trim();
        csvRecord.expiry = curruntRecord[1].trim();
        csvRecord.securitycode = curruntRecord[2].trim();
        csvRecord.isUsed = 0;
